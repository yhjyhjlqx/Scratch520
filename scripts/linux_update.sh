...
function check_tools() {
    local tools=("unzip" "curl")
    for tool in "${tools[@]}"; do
        if ! command -v "$tool" &> /dev/null; then
            echo "正在安装 $tool..."
            if command -v apt-get &> /dev/null; then
                apt-get update -qq && apt-get install -yq "$tool"
...
echo "刷新DNS缓存..."
if systemctl restart nscd &> /dev/null || systemctl restart NetworkManager &> /dev/null; then
    echo "DNS缓存已刷新"
else
    echo "无法刷新DNS缓存" >&2
fi
